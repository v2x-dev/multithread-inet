...
void PhysicalEnvironment::visitObjects(const IVisitor *visitor, const LineSegment& lineSegment) const
{
    if (objectCache)
        ...
        int threads_num = (int)par("numberOfThreads");
        ...
        //Run parallel code
            ...
            int range = objects.size()/threads_num;

            pthread_attr_t attr;
            pthread_attr_init(&attr);
            pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_JOINABLE);
            pthread_t * threads[threads_num];
            visitor_data * threads_data[threads_num];

            for (int i = 0; i < threads_num; i++) {
                threads_data[i] = new visitor_data();
                visitor_data &v = *threads_data[i];

                v.startIndex = i*range;
                if (i < threads_num-1)
                    v.offset = range;
                else
                    v.offset = objects.size()-v.startIndex;
                v.objects = &objects;
                v.visitor = visitor;

                threads[i] = new pthread_t();
                int rc = pthread_create(threads[i], NULL, visitorDoVisit, &v);

                if (rc) {
                    throw std::runtime_error("Thread in PhysicalEnvironment::visitObjects can not be created.");
                }
            }
            ...
            for (int i = 0; i < threads_num; i++) {
                ...
                if (rc) {
                    throw std::runtime_error("Can not join the thread " + std::to_string(i) + " in PhysicalEnvironment::visitObjects.");
                }
            }
        }
    }
...
void * PhysicalEnvironment::visitorDoVisit(void *data) {
    if (data == nullptr) {
        throw std::invalid_argument("void * PhysicalEnvironment::visitorDoVisit : input data is nullptr!");
        pthread_exit(NULL);
    }

    visitor_data *data_passed = (struct visitor_data *)data;
    auto &objects = data_passed->objects;
    const auto objectsStart = objects->begin() + data_passed->startIndex;
    const auto objectsEnd = objectsStart + data_passed->offset;

    for (auto elem = objectsStart; elem < objectsEnd; elem++) {
        data_passed->visitor->visit(*elem);
    }
