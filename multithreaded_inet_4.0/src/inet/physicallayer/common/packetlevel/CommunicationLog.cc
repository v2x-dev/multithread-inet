...
void CommunicationLog::writeReception(const IRadio *receiver, const ISignal *signal)
{
    ...
    W rssiWatts = flatReception->computeMinPower(reception->getStartTime(), reception->getEndTime());
    //    double rssidBm = rssiWatts.value
    //    inet::math::mW2dBm(rssiWatts);
    //    W rssiValue = W(10);
    ...
    output << "R " << receiverRadio->getFullPath() << " " << reception->getReceiver()->getId() << " "
           ...
           << "RSSI " << inet::math::mW2dBm(rssiWatts.get()) << " dBm" << endl;
}
