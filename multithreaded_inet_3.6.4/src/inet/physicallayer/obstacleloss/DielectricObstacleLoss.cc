...
{
    double objectLoss = obstacleLoss->computeObjectLoss(check_and_cast<const IPhysicalObject *>(object), frequency, transmissionPosition, receptionPosition);
    double oldValueOfTotalLoss = totalLoss.load();
    while (!totalLoss.compare_exchange_weak(oldValueOfTotalLoss, oldValueOfTotalLoss*objectLoss)){}
}
