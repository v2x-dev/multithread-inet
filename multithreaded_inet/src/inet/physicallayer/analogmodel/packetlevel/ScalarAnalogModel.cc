...
//    W receptionPower = computeReceptionPower(receiverRadio, transmission, arrival);
    ...
    W receptionPower;
    Coord startP = transmission->getStartPosition();
    Coord endP = arrival->getStartPosition();
    double distance = sqrt(pow(endP.x-startP.x,2) + pow(endP.y-startP.y,2) + pow(endP.z-startP.z,2));
    double distanceBoundary = (double)par("distanceBoundary");
